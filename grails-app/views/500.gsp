<!DOCTYPE html>
<html>
<head>
    <meta name="layout" content="first"/>
    <r:require modules="company,first_deprecated"/>

</head>


<body>

<g:render template="/shared/header"></g:render>

<section id="content">
    <div class="main padder">
        <div class="row">
            <div class="col-lg-4 col-lg-offset-4">
                <div class="text-center m-b-large">
                    <h1 class="h text-white">500</h1>
                </div>
                <div class="list-group m-b-small">
                    <a href="/" target="_self" class="list-group-item">
                        <i class="fa fa-chevron-right"></i>
                        <i class="fa fa-home"></i> Ana sayfaya dön
                    </a>
                    <a href="https://www.facebook.com/groups/kimlik.io/" target="_blank" class="list-group-item">
                        <i class="fa fa-chevron-right"></i>
                        <i class="fa fa-question"></i> Bu hatta hakkında bize yazmak isterseniz...
                    </a>
                    <a href="#" class="list-group-item">
                        <i class="fa fa-chevron-right"></i>
                        <span class="badge">info [at] kimlik.io</span>
                        <i class="fa fa-envelope-o"></i> Ouuuugk!
                    </a>
                </div>
            </div>
        </div>
    </div>
</section>

<g:render template="/shared/footer"/>


</body>

</html>

